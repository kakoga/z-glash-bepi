    ...
    <div class="container">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav navbar-right" id="nav" itemscope="" itemtype="http://schema.org/SiteNavigationElement">
				...
				{{ $subnav = 1 }}
				{{ if {$subindex} == 1 }}
				<li class=" dropdown"><a class="{{ $name }} dropdown-toggle" href="{{if {$exlink} }}{{$exlink}}{{else}}{{ truepath($id) }}{{end-if}}" title="{{ $name }}" itemprop="url"><span itemprop="name">{{ $name }}</span><b class="caret hide-on-sm"></b></a>
					<ul class="{{ $name }} dropdown-menu" id="" itemscope="">
						{{ end-if }}
						<li class="">
							<a class="{{ subnl.name }} dropdown-toggle hide-on-sm" href=" {{if {subnl.external_link} }}{{subnl.external_link}}{{else}}{{ truepath({subnl.internal_nav_link}) }}{{end-if}}" title="{{ subnl.name }}" itemprop="url"><span itemprop="name">{{ subnl.name }}</span>
							</a>
						</li>
						{{ end-each }}
				...
				{{ else }}
				<li class=""><a class="{{ $name }} dropdown-toggle" href="{{if {$exlink} }}{{$exlink}}{{else}}{{ truepath({$id}) }}{{end-if}}" title="{{ $name }}" itemprop="url"><span itemprop="name">{{ $name }}</span></a></li>
				{{ end-if }}
